...
/**
 * Script to find wallets that have claimed tokens from November vesting schedules
 ...
 *   OUTPUT_FILE - Output JSON file path (default: scripts/data/november-claimed-with-balance.json)
...
const OUTPUT_FILE = process.env.OUTPUT_FILE || path.join(__dirname, "data/november-claimed-with-balance.json");
...
async function main() {
    console.log("🚀 Starting November claimed wallets with balance query...\n");
    ...
    try {
        ...
        // Second pass: filter and process November schedules
        for (const event of allScheduleEvents) {
            if (!("args" in event)) continue;
        ...
        // Map schedule IDs to November schedules for quick lookup
        const novemberScheduleMap = new Map<string, NovemberScheduleData>();
        for (const schedule of novemberSchedules) {
            novemberScheduleMap.set(schedule.scheduleId, schedule);
        }
        
        // Filter claim events to only November schedules
        const novemberClaimEvents: Array<{ event: EventLog; schedule: NovemberScheduleData }> = [];
        for (const event of claimEvents) {
            if (!("args" in event)) continue;
            const args = event.args as any[];
            const scheduleId = args[0].toString(); // scheduleId is first indexed parameter
            const schedule = novemberScheduleMap.get(scheduleId);
            if (schedule) {
                novemberClaimEvents.push({ event, schedule });
            }
        }
        
        console.log(`✅ Found ${novemberClaimEvents.length} claims from November schedules\n`);
        ...
        const uniqueWallets = new Map<string, Array<{ event: EventLog; schedule: NovemberScheduleData }>>();
        for (const item of novemberClaimEvents) {
        ...
        console.log(`📦 Processing ${novemberClaimEvents.length} claims from ${uniqueWallets.size} unique wallets...\n`);
        ...
        const claimedWalletsWithBalance: NovemberClaimedWalletData[] = [];
        const claimedWalletsNoBalance: NovemberClaimedWalletData[] = [];
        
        // Process each claim event
        let processed = 0;
        for (let i = 0; i < novemberClaimEvents.length; i += CONCURRENT_REQUESTS) {
            const batch = novemberClaimEvents.slice(i, i + CONCURRENT_REQUESTS);
            ...
            const batchPromises = batch.map(async (item) => {
                try {
                    ...
                    const walletData: NovemberClaimedWalletData = {
            ...
            const validResults = batchResults.filter((result): result is NovemberClaimedWalletData => result !== null);
            ...
            if (processed % 1000 === 0 || processed === novemberClaimEvents.length) {
                const elapsed = ((Date.now() - step3StartTime) / 1000).toFixed(1);
                const progress = ((processed / novemberClaimEvents.length) * 100).toFixed(1);
                console.log(`   📊 Processing ${processed}/${novemberClaimEvents.length} (${progress}%) - Found ${claimedWalletsWithBalance.length} with balance, ${claimedWalletsNoBalance.length} without balance - ${elapsed}s elapsed`);
            }
        }
        
        const step3Elapsed = ((Date.now() - step3StartTime) / 1000).toFixed(1);
        console.log(`✅ Processed ${novemberClaimEvents.length} claims in ${step3Elapsed}s`);
