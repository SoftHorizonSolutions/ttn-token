...
/**
 * Script to find wallets that have December vesting schedules but haven't claimed their tokens
 * 
 * Usage: 
 *   # For Ethereum mainnet (where schedules were created):
 *   npx hardhat run scripts/query-december-unclaimed.ts --network mainnet
 * 
 *   # For Base mainnet:
 *   npx hardhat run scripts/query-december-unclaimed.ts --network base
 * 
 ...
 *   OUTPUT_FILE - Output JSON file path (default: scripts/data/december-unclaimed-wallets.json)
...
const OUTPUT_FILE = process.env.OUTPUT_FILE || path.join(__dirname, "data/december-unclaimed-wallets.json");
...
async function main() {
    console.log("🚀 Starting December unclaimed wallets query...\n");
    const startTimeDate = new Date(DECEMBER_START_TIME * 1000).toISOString();
    console.log(`📅 December Start Time: ${DECEMBER_START_TIME} (${startTimeDate})\n`);
    ...
    try {
        ...
        // (December schedules were likely created recently, but we want to be thorough)
        ...
        if (allScheduleEvents.length > 0) {
            ...
            const targetDate = new Date(DECEMBER_START_TIME * 1000).toISOString();
            console.log(`\n🎯 Looking for start time: ${DECEMBER_START_TIME} (${targetDate})\n`);
        }
        ...
        // Second pass: filter and process December schedules using cached block data
        for (const event of allScheduleEvents) {
            if (!("args" in event)) continue;
            ...
            // Filter for December schedules only
            if (startTime !== DECEMBER_START_TIME) {
                continue;
            }
        ...
        const unclaimedWallets: DecemberWalletData[] = [];
        ...
        async function processSchedule(schedule: DecemberScheduleData): Promise<DecemberWalletData | null> {
            try {
        ...
        for (let i = 0; i < decemberSchedules.length; i += CONCURRENT_REQUESTS) {
            const batch = decemberSchedules.slice(i, i + CONCURRENT_REQUESTS);
            ...
            const validResults = batchResults.filter((result): result is DecemberWalletData => result !== null);
            ...
            if (processed % 1000 === 0 || processed === decemberSchedules.length) {
                const elapsed = ((Date.now() - step3StartTime) / 1000).toFixed(1);
                const progress = ((processed / decemberSchedules.length) * 100).toFixed(1);
                const rate = (processed / ((Date.now() - step3StartTime) / 1000)).toFixed(1);
                console.log(`   📊 Processing schedule ${processed}/${decemberSchedules.length} (${progress}%) - ${rate} schedules/sec - ${errors} errors - ${elapsed}s elapsed`);
            }
        }
        
        console.log(`\n✅ Processed ${decemberSchedules.length} December schedules`);
        console.log(`✅ Found ${unclaimedWallets.length} wallets with unclaimed December tokens\n`);
        
        // Step 4: Generate summary and save results
        console.log("=".repeat(80));
        console.log("📊 SUMMARY");
        console.log("=".repeat(80));
        console.log(`Total December Schedules: ${decemberSchedules.length}`);
        console.log(`Wallets with Unclaimed December Tokens: ${unclaimedWallets.length}`);
        ...
        console.log(`Total Unclaimed December Tokens: ${totalUnclaimed.toLocaleString(undefined, {
        ...
        // Display top 10
        console.log("🏆 Top 10 Wallets by Unclaimed December Amount:");
        console.log("-".repeat(80));
